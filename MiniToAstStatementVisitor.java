   ...
   extends MiniBaseVisitor<Statement>
{
   ...
   public Statement visitStatementList(MiniParser.StatementListContext ctx)
   {
      ...
      return new BlockStatement(statements, ctx.getStart().getLine());
   }
   ...
   private Expression visitLvalueNested(MiniParser.LvalueContext ctx) {
      ...
      } else if (ctx instanceof MiniParser.LvalueIndexContext) {
          MiniParser.LvalueIndexContext lctx = (MiniParser.LvalueIndexContext) ctx;
          return new IndexExpression(lctx.getStart().getLine(), visitLvalueNested(lctx.lvalue()), expressionVisitor.visit(lctx.expression()));
      }
